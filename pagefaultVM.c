...
int pagefault(char *vaddress)
{
    ...
    if (i >= RESIDENTSETSIZE)
    {
        ...
        if (vframe == -1)
        {
            printf("[ERROR] - No hay frames virtuales en memoria secundaria.\n");
            return (-1);
        }
    ...
    if (frame == -1)
    {
        printf("[ERROR] - No hay marcos físicos en el sistema.\n");
...
int getfifo(){
    ...
    int pag_a_expulsar = 0;
        for(int i = 0; i < ptlr;i++){
            if((ptbr + i)->tarrived <= time && (ptbr + i)->presente){
                time = (ptbr + i)->tarrived;
                pag_a_expulsar = i;
            }
        }
