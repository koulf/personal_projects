...
int searchvirtualframe()
{
    int i;
    // Busca un marco libre en el sistema
    for(i=systemframetablesize;i<systemframetablesize*2;i++)
        if(!systemframetable[i].assigned)
        {
            systemframetable[i].assigned=1;
            break;
        }
    if(i<systemframetablesize*2)
        systemframetable[i].assigned=1;
    else
        i=-1;
    return(i);
...
int getfifo()
{
    long x = __LONG_MAX__, y = -1;
    for(int i = 0; i < ptlr; i++)
        if((ptbr+i)->presente && (ptbr+i)->tarrived <= x)
        {
            x = (ptbr+i)->tarrived;
            y = i;
        }
...
int pagefault(char *vaddress)
{
    ...
    if((ptbr+pag_del_proceso)->framenumber > 11)
    {
